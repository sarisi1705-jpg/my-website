...
function ProductsMenu({ mobile = false, active = false, onNavigate }: { mobile?: boolean; active?: boolean; onNavigate?: () => void }) {
  ...
  return <div className={`products-menu${mobile ? " products-menu--mobile" : ""}`} ref={menuRef}>
    ...
    {open && <div className="products-dropdown" role="menu" aria-label="أقسام المنتجات">
      {productCategories.map(category => <Link key={category.href} href={category.href} role="menuitem" onClick={() => { setOpen(false); onNavigate?.(); }}>
  ...
  return <header className="site-header sticky top-0 z-40 border-b border-[#dfe7f2] bg-white/92 backdrop-blur-xl">
    <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-4 sm:px-8 lg:px-12">
      ...
      <nav className="hidden items-center gap-5 lg:flex" aria-label="التنقل الرئيسي">
        ...
        {navigation.filter(item => item.key !== "home").map(item => <Link key={item.key} className={`nav-link${resolvedActive === item.key ? " nav-link--active" : ""}`} href={item.href} aria-current={resolvedActive === item.key ? "page" : undefined}>{item.label}</Link>)}
      </nav>
      <div className="header-actions">
        ...
        <Button asChild className="header-quote hidden rounded-xl bg-[#1258dc] px-4 2xl:inline-flex"><Link href="/contact">اطلب عرض السعر <ChevronLeft /></Link></Button>
    ...
    {mobileMenu && <nav className="mobile-nav lg:hidden" aria-label="التنقل الرئيسي للهاتف">
      ...
      {navigation.filter(item => item.key !== "home").map(item => <Link key={item.key} className={resolvedActive === item.key ? "mobile-nav--active" : ""} href={item.href} aria-current={resolvedActive === item.key ? "page" : undefined} onClick={() => setMobileMenu(false)}>{item.label}</Link>)}
    </nav>}
  </header>;
}
