...
function ProductsMenu({ mobile = false, active = false, onNavigate }: { mobile?: boolean; active?: boolean; onNavigate?: () => void }) {
  ...
  return <div className={`products-menu${mobile ? " products-menu--mobile" : ""}`} ref={menuRef}>
    ...
    {open && <div className="products-dropdown" role="menu" aria-label="أقسام المنتجات">
      {productCategories.map(category => <a target="_top" key={category.href} href={category.href} role="menuitem" onClick={mobile ? (event) => { event.preventDefault(); window.location.assign(category.href); } : undefined}>
...
export function SiteHeader({ active = "home" }: { active?: SitePage }) {
  ...
  return <header className="site-header sticky top-0 z-40 border-b border-[#dfe7f2] bg-white/92 backdrop-blur-xl">
    <div className="mx-auto flex h-[76px] max-w-[1440px] items-center justify-between px-4 sm:px-8 lg:px-12">
      <a target="_top" href="/" className="brand-lockup" aria-label="SSPS — الصفحة الرئيسية"><span className="brand-symbol"><Printer aria-hidden="true" /></span><span><strong>SSPS</strong><small>حلول لأنظمة الحلول والطباعة</small></span></a>
      <nav className="hidden items-center gap-5 lg:flex" aria-label="التنقل الرئيسي">
        <a target="_top" className={`nav-link${resolvedActive === "home" ? " nav-link--active" : ""}`} href="/" aria-current={resolvedActive === "home" ? "page" : undefined}>الرئيسية</a>
      ...
      <div className="header-actions">
        ...
        <Button asChild className="header-quote hidden rounded-xl bg-[#1258dc] px-4 2xl:inline-flex"><a target="_top" href="/contact">اطلب عرض السعر <ChevronLeft /></a></Button>
    ...
    {mobileMenu && <nav className="mobile-nav lg:hidden" aria-label="التنقل الرئيسي للهاتف">
      <a target="_top" className={resolvedActive === "home" ? "mobile-nav--active" : ""} href="/" aria-current={resolvedActive === "home" ? "page" : undefined} onClick={(event) => { event.preventDefault(); window.location.assign("/"); }}>الرئيسية</a>
      <ProductsMenu mobile active={resolvedActive === "products"} onNavigate={() => setMobileMenu(false)} />
      {navigation.filter(item => item.key !== "home").map(item => <a target="_top" key={item.key} className={resolvedActive === item.key ? "mobile-nav--active" : ""} href={item.href} aria-current={resolvedActive === item.key ? "page" : undefined} onClick={(event) => { event.preventDefault(); window.location.assign(item.href); }}>{item.label}</a>)}
    </nav>}
  </header>;
}
