...
export default async function ContactPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const match = Number.isInteger(productId) ? products.find(item => item.id === productId) : undefined;
  const product: InquiryProduct | undefined = match && { id: match.id, name: match.name, brand: match.brand, model: match.model };
